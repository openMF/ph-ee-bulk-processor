...
public class AccountLookupCallbackWorker extends BaseWorker{
    ...
    public void setup(){
        ...
        zeebeClient.newWorker()
                .jobType(ACCOUNT_LOOKUP_CALLBACK.getValue())
                .handler((client, job) -> {
                    logger.info("Job '{}' started from process '{}' with key {}", job.getType(), job.getBpmnProcessId(), job.getKey());
                    Map<String, Object> existingVariables = job.getVariablesAsMap();
                    existingVariables.put(ORIGIN_CHANNEL_REQUEST,existingVariables.get(CHANNEL_REQUEST));
                    TransactionChannelRequestDTO transactionChannelRequestDTO = objectMapper.readValue((String) existingVariables.get(CHANNEL_REQUEST), TransactionChannelRequestDTO.class);
