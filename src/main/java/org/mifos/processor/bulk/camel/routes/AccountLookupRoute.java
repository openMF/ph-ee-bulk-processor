...
public class AccountLookupRoute extends BaseRouteBuilder{
    ...
    public void configure() throws Exception {
        ...
        from("rest:POST:/accountLookup/Callback")
                .log(LoggingLevel.DEBUG, "######## -> ACCOUNT LOOKUP CALLBACK")
        ...
        from("direct:send-account-lookup")
                ...
                .setHeader(
                        Exchange.HTTP_QUERY,
                        simple(new StringBuilder().append(PAYEE_IDENTITY).append("=${exchangeProperty.")
                                .append(PAYEE_IDENTITY).append("}&").append(PAYMENT_MODALITY)
                                .append("=${exchangeProperty.").append(PAYMENT_MODALITY)
                                        .append("}&").append("requestId=${exchangeProperty.requestId}").toString()
                        )
                )
                .setProperty(HOST, simple("{{identity-account-mapper.hostname}}"))
                .setProperty(ENDPOINT, simple("identityAccountMapper/accountLookup/"))
